...
const VALID_CONDITION_TYPES = ['min_transaction_amount', 'min_qty', 'specific_product', 'specific_category', 'member_type'];
...
const getPromotionById = async (id) => {
  ...
  const conditions = await promotionManagementModel.findConditionsByPromotionId(id);
...
const deletePromotion = async (id) => {
  ...
  if (usage > 0) throw new Error('Cannot delete promotion that has been used in transactions');
...
const addCondition = async (promotionId, data) => {
  ...
  if (!VALID_CONDITION_TYPES.includes(data.condition_type)) {
    throw new Error(`Invalid condition_type. Valid values: ${VALID_CONDITION_TYPES.join(', ')}`);
  }

  if (!VALID_OPERATORS.includes(data.operator)) {
    throw new Error(`Invalid operator. Valid values: ${VALID_OPERATORS.join(', ')}`);
  }

  if (['specific_product', 'specific_category'].includes(data.condition_type) && !data.target_id) {
    throw new Error('target_id is required for specific_product and specific_category conditions');
  }
...
const removeCondition = async (promotionId, conditionId) => {
  ...
  const condition = await promotionManagementModel.findConditionById(conditionId);
  if (!condition) throw new Error('Condition not found');
  if (condition.promotion_id !== promotionId) throw new Error('Condition does not belong to this promotion');
...
const addReward = async (promotionId, data) => {
  ...
  if (!VALID_REWARD_TYPES.includes(data.reward_type)) {
    throw new Error(`Invalid reward_type. Valid values: ${VALID_REWARD_TYPES.join(', ')}`);
  }
  ...
  if (data.reward_type !== 'free_item') {
    ...
    if (!VALID_DISCOUNT_MODES.includes(data.discount_mode)) {
      throw new Error(`Invalid discount_mode. Valid values: ${VALID_DISCOUNT_MODES.join(', ')}`);
    }
  }

  if (data.reward_type === 'free_item') {
    if (!data.free_variant_id) throw new Error('free_variant_id is required for free_item reward');
    if (!data.free_qty || data.free_qty <= 0) throw new Error('free_qty must be greater than 0');
  }
...
const removeReward = async (promotionId, rewardId) => {
  ...
  if (reward.promotion_id !== promotionId) throw new Error('Reward does not belong to this promotion');
