...
const evaluateConditions = (conditions, context) => {
  for (const condition of conditions) {
    const { condition_type, operator, value, target_type, target_id } = condition;
    let actual = null;
    ...
    if (condition_type === 'min_transaction_amount') {
      ...
      const found = context.items.some(item => item.category_id === target_id);
      if (!found) return false;
      continue;
    } else if (condition_type === 'member_type') {
      actual = context.customerMemberType ?? 'regular';
    }
  ...
  if (reward.reward_type === 'discount_percent') {
    if (reward.discount_mode === 'per_transaction') {
      ...
      for (const item of items) {
        ...
        result.itemDiscounts.push({ variantId: item.product_variant_id, discount });
        result.discountAmount += discount;
      }
    }
    ...
    if (reward.discount_mode === 'per_transaction') {
      ...
      for (const item of items) {
        const discount = Math.min(parseFloat(reward.discount_value), item.subtotal);
        result.itemDiscounts.push({ variantId: item.product_variant_id, discount });
        result.discountAmount += discount;
      }
    }
...
const applyPromotions = async (promoCodes, voucherCodes, context) => {
  ...
  for (const code of allRequestedCodes) {
    ...
    if (promotion.usage_limit > 0 && promotion.usage_count >= promotion.usage_limit) continue;

    const conditions = await promotionModel.findConditionsByPromotionId(promotion.id);
